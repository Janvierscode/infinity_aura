...
import { getLatestBusinessIdeas, getServices } from "@/lib/content";

export default async function HomePage() {
  const [services, ideas] = await Promise.all([getServices(), getLatestBusinessIdeas(3)]);
  ...
  return (
    <>
      ...
      <section className="section ideas-home-section"><div className="container"><div className="section-heading"><div><span className="section-label">Business Ideas</span><h2>Start with an opportunity you can understand.</h2></div><Link className="text-link" href="/ideas">Explore all ideas <ArrowRight size={16} /></Link></div>{ideas.length ? <div className="idea-grid">{ideas.map((idea, index) => <IdeaCard key={idea.id} idea={idea} priority={index === 0} />)}</div> : <div className="empty-state"><h2>New opportunities are on the way.</h2><p>The first practical business ideas are being prepared for the community.</p><Link className="button button-secondary" href="/account/signup">Create your account</Link></div>}</div></section>
